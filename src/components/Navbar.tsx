...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
      <header
        id="app-header-nav"
        className="sticky top-0 z-40 w-full bg-[#FAF7F9]/80 backdrop-blur-md border-b border-pink-100/60 transition-all duration-200"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-18 flex items-center justify-between gap-4">
          ...
          {/* Navigation Links in Editorial Style */}
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
            <button
              id="nav-home-btn"
              onClick={() => onNavigate("dashboard")}
              className={`text-xs uppercase tracking-[0.15em] font-bold transition-all cursor-pointer ${
                currentView === "dashboard"
                  ? "border-b-2 border-pink-400 pb-1 text-purple-950 font-extrabold"
                  : "text-purple-400/80 hover:text-purple-950"
              }`}
            >
              Home
            </button>
            ...
            <button
              ...
              className={`text-xs uppercase tracking-[0.15em] font-bold transition-all cursor-pointer flex items-center gap-1.5 px-3 py-1 rounded-full ${
            ...
            <button
              ...
              className={`text-xs uppercase tracking-[0.15em] font-bold transition-all cursor-pointer flex items-center gap-1 ${
                currentView === "globe"
                  ? "border-b-2 border-pink-400 pb-1 text-purple-950 font-extrabold"
                  : "text-purple-400/80 hover:text-purple-950"
              }`}
            ...
            <button
              ...
              className={`text-xs uppercase tracking-[0.15em] font-bold transition-all cursor-pointer flex items-center gap-1.5 px-3 py-1 rounded-full ${
                currentView === "ayra"
                  ...
                  : "text-purple-600 hover:text-purple-950 bg-pink-100/60 hover:bg-pink-100 border border-pink-200/60"
              }`}
            >
              <span>💜</span>
              <span>AYRA</span>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <AmbientSoundControl variant="compact" />

            {/* Security & Isolation Probe Trigger Button */}
            <button
            ...
            <button
              ...
              className="flex items-center gap-2 pl-2 pr-3 py-1 rounded-full bg-white/80 hover:bg-pink-50 border border-pink-200/60 transition-colors cursor-pointer"
            >
              ...
              <span className="text-xs font-serif italic text-purple-950 max-w-[80px] truncate hidden sm:inline">
                {userProfile.name}
              </span>
            </button>
          </div>
        ...
        <div className="flex lg:hidden overflow-x-auto px-4 py-2 border-t border-pink-100/60 gap-4 text-xs font-bold uppercase tracking-wider text-purple-900/70 bg-white/60">
          <button
            onClick={() => onNavigate("dashboard")}
            className={`whitespace-nowrap ${currentView === "dashboard" ? "text-pink-600 underline font-extrabold" : ""}`}
          >
            Home
          </button>
          <button
            onClick={() => onNavigate("diary-book")}
            className={`whitespace-nowrap flex items-center gap-1 ${currentView === "diary-book" ? "text-white bg-pink-600 px-2.5 py-0.5 rounded-full font-extrabold" : "text-pink-700 font-bold"}`}
          >
            <span>📖</span>
            <span>Diary Book</span>
          </button>
          <button
            onClick={() => onNavigate("all-entries")}
            className={`whitespace-nowrap ${currentView === "all-entries" ? "text-pink-600 underline font-extrabold" : ""}`}
          >
            All Journals
          </button>
          <button
            onClick={() => onNavigate("globe")}
            className={`whitespace-nowrap ${currentView === "globe" ? "text-pink-600 underline font-extrabold" : ""}`}
          >
            🌍 Globe
          </button>
          <button
            onClick={() => onNavigate("inbox")}
            className={`whitespace-nowrap ${currentView === "inbox" ? "text-pink-600 underline font-extrabold" : ""}`}
          >
            📥 Inbox
          </button>
          <button
            onClick={() => onNavigate("ayra")}
            className={`whitespace-nowrap flex items-center gap-1 ${currentView === "ayra" ? "text-purple-950 font-extrabold bg-pink-100 px-2 py-0.5 rounded-full" : "text-purple-700"}`}
          >
            <span>💜</span>
            <span>AYRA</span>
          </button>
